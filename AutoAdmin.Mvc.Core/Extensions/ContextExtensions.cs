...
namespace AutoAdmin.Mvc.Core.Extensions
{
    public static class ContextExtensions
    {
        ...
        {
            return /*(DbSet<object>)*/ ctx.GetType().GetProperty(tableName).GetValue(ctx);
        }
        ...
        /// <returns></returns>
        public static object CopyFrom(this object to, IFormCollection from, string tableName)
        {
            foreach (var property in to.GetType().GetProperties())
            {
                try
                {
                    if (from[property.Name].Count == 0) continue;
                    ...
                    switch (relation)
                    {
                        ...
                        case Relation.ManyToMany:
                            ...
                            foreach (var id in from[property.Name])
